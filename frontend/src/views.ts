  ...
  return `
    <div class="admin-overview-stack" style="display: grid; gap: 24px;">
      ...
      <div class="top-students-card" style="background: var(--surface); padding: 20px; border-radius: 10px; border: 1px solid var(--line); box-shadow: var(--shadow-subtle);">
        <h3 style="margin-bottom: 6px; font-size: 16px; color: var(--text-primary);">Kiểm tra tính nhất quán khi chấm</h3>
        <p style="margin: 0 0 12px; color: var(--text-secondary); font-size: 13px;">Báo cáo này không kết luận ai “thiên vị”. Nó chỉ chỉ ra các mẫu điều chỉnh điểm cần cùng nhau rà soát và hiệu chuẩn rubric.</p>
        <div class="grading-review-guide" role="note" aria-label="Cách đọc báo cáo nhất quán chấm điểm">
          <div><strong>1. So sánh công bằng</strong><span>Chỉ đối chiếu các lượt review trong cùng scenario và cùng độ khó.</span></div>
          <div><strong>2. Đọc chênh lệch</strong><span><b>Điều chỉnh = điểm sau review − điểm AI.</b> Dấu + là nâng điểm; dấu − là hạ điểm.</span></div>
          <div><strong>3. Khi có cờ</strong><span>“Cần hiệu chuẩn” nghĩa là cần xem lại rubric và bằng chứng, không phải kết luận về cá nhân.</span></div>
        </div>
        <div style="overflow-x: auto;">
          <table style="width: 100%; border-collapse: collapse; font-size: 13px; text-align: left;">
            <thead><tr style="border-bottom: 1px solid var(--line); color: var(--text-secondary);"><th style="padding: 10px;">Người review</th><th style="padding: 10px; text-align: center;">Mẫu review</th><th style="padding: 10px; text-align: center;">Điểm AI TB</th><th style="padding: 10px; text-align: center;">Điểm sau review TB</th><th style="padding: 10px; text-align: center;">Điều chỉnh TB</th><th style="padding: 10px; text-align: center;">Kết quả kiểm tra</th></tr></thead>
            <tbody>
              ${(admin.gradingReview?.reviewers ?? []).length === 0 ? '<tr><td colspan="6" style="padding: 20px; text-align: center; color: var(--muted);">Chưa có dữ liệu điều chỉnh điểm để phân tích.</td></tr>' : (admin.gradingReview?.reviewers ?? []).map(item => `
                <tr style="border-bottom: 1px solid var(--line-subtle);"><td style="padding: 10px; font-weight: 600; color: var(--text-primary);">${escapeHtml(item.lecturerName)}</td><td style="padding: 10px; text-align: center;">${item.reviewCount}</td><td style="padding: 10px; text-align: center;">${item.averageAiScore}%</td><td style="padding: 10px; text-align: center;">${item.averageFinalScore}%</td><td style="padding: 10px; text-align: center; font-weight: 600; color: ${item.averageAdjustment > 0 ? 'var(--pastel-green-text)' : item.averageAdjustment < 0 ? 'var(--accent-rose)' : 'var(--text-secondary)'};">${item.averageAdjustment > 0 ? '+' : ''}${item.averageAdjustment} điểm</td><td style="padding: 10px; text-align: center;">${item.requiresReview ? '<span style="color: var(--accent-rose); font-weight: 700;" title="Cùng xem lại rubric và bằng chứng của các lượt chấm tương đương.">Cần hiệu chuẩn</span>' : item.hasSufficientData ? '<span style="color: var(--pastel-green-text);" title="Chưa có mẫu điều chỉnh vượt ngưỡng đã đặt.">Ổn định trong ngưỡng</span>' : '<span style="color: var(--text-secondary);" title="Chưa đủ lượt review để suy luận.">Chưa đủ dữ liệu</span>'}</td></tr>`).join('')}
            </tbody>
          </table>
        </div>
        <small style="display:block; margin-top: 10px; color: var(--muted);">Ngưỡng áp dụng khi một nhóm có ít nhất ${admin.gradingReview?.methodology.minimumReviews ?? 5} lượt review: cờ nếu điều chỉnh trung bình từ ±${admin.gradingReview?.methodology.meanAdjustmentThreshold ?? 15} điểm, hoặc ít nhất một nửa số lượt lệch từ ${admin.gradingReview?.methodology.highAdjustmentThreshold ?? 25} điểm.</small>
        ${(admin.gradingReview?.cohorts ?? []).length > 0 ? `<details class="grading-review-cohorts"><summary>Xem nhóm so sánh theo scenario và độ khó (${(admin.gradingReview?.cohorts ?? []).length})</summary><div>${(admin.gradingReview?.cohorts ?? []).map(item => `<span>${escapeHtml(item.lecturerName)} · ${escapeHtml(item.scenarioTitle)} · ${escapeHtml(item.difficulty)} · n=${item.reviewCount} · ${item.averageAdjustment > 0 ? '+' : ''}${item.averageAdjustment} điểm · ${item.requiresReview ? 'Cần hiệu chuẩn' : item.hasSufficientData ? 'Trong ngưỡng' : 'Chưa đủ dữ liệu'}</span>`).join('')}</div></details>` : ''}
      </div>
      <!-- Charts 2x2 Grid -->
